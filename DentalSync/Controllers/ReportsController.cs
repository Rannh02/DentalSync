...
namespace DentalSync.Controllers
{
    ...
    public class ReportsController : Controller
    {
            ...
            int invoicePage = 1)
        {
            ...
            var invAlerts = await _db.Set<Supply>()
            ...
            var apptStatuses = await _db.Set<Appointment>()
                .Where(a => a.CreatedAt >= dateFrom && a.CreatedAt <= dateTo)
                .GroupBy(a => a.Status)
                .Select(g => new { Status = g.Key, Count = g.Count() })
                .ToListAsync();

            var apptStats = new AppointmentStatsViewModel
            {
            ...
            // ── Inventory stats ───────────────────────────────────────────────
            var totalSupplies = await _db.Set<Supply>().CountAsync();
            var lowStock      = await _db.Set<Supply>().CountAsync(s => s.MinimumStock.HasValue && s.Quantity > 0 && s.Quantity <= s.MinimumStock);
            var outOfStock    = await _db.Set<Supply>().CountAsync(s => s.Quantity == 0);
            var expiring      = await _db.Set<Supply>().CountAsync(s => s.ExpirationDate.HasValue && s.ExpirationDate <= today30 && s.ExpirationDate >= todayDo);
            ...
            var vm = new ReportsViewModel
            {
                ...
                Financial = new FinancialStatsViewModel
                {
                    TotalRevenue   = payments,
                ...
                InventoryStats = new InventoryStatsViewModel
                {
